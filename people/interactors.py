...
class BlockInteractor:
    ...
    def __init__(self, permissions_validator, block_repo, experience_repo,
                 profile_repo, save_unsave_experience_interactor):
    ...
    def set_params(self, logged_person_id, target_username):
        self.logged_person_id = logged_person_id
    ...
    def execute(self):
        ...
        target_id = self.profile_repo.get_profile(username=self.target_username,
                                                  logged_person_id=self.logged_person_id).person_id

        if self.logged_person_id == target_id:
            raise ConflictException(source='person', code='conflict', message='Cannot block yourself')

        if self.block_repo.block_exists(creator_id=self.logged_person_id, target_id=target_id):
            return True
        ...
        for experience in saved_experiences:
            if experience.author_id == target_id:
        ...
        return self.block_repo.block(creator_id=self.logged_person_id, target_id=target_id)
